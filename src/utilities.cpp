...
double areaOfTriangle(double a, double b, double c) {
    ...
    // Sides must obey triangle inequality
...
double areaOfTriangle(double x1, double y1, double x2, double y2, double x3, double y3) {
	double s1 = distanceBetween(x1, y1, x2, y2);
    double s2 = distanceBetween(x2, y2, x3, y3);
    double s3 = distanceBetween(x3, y3, x1, y1);

	return areaOfTriangle(s1, s2, s3);
}
...
void pointOnUnitCircle(double angleRads, double& x, double& y) {
    double r = normalizeRadians(angleRads);
    x = glm::cos(r);
    y = glm::sin(r);
}
...
dvec2 pointOnCircle(const dvec2& center, double R, double angleRads) {
    double r = normalizeRadians(angleRads);
    double x = (R * glm::cos(r)) + center.x;
    double y = (R * glm::sin(r)) + center.y;
    dvec2 result = {x, y};
    return result;
}
...
double directionInRadians(const dvec2& referencePt, const dvec2& targetPt) {
	double dx = targetPt.x - referencePt.x;
    double dy = targetPt.y - referencePt.y;
...
double map(double x, double fromLo, double fromHi, double toLow, double toHigh) {
	double ratio = (x - fromLo) / (fromHi - fromLo);
    double newX = (ratio * (toHigh - toLow)) + toLow;
    return newX;
}
...
vector<double> quadratic(double A, double B, double C) {
    double roots[2] = {0, 0};
    int rootCount = quadratic(A, B, C, roots);
...
int quadratic(double A, double B, double C, double roots[2]) {
    ...
    if (discriminant > 0) {
        ...
        roots[0] = glm::min(x1, x2);
        roots[1] = glm::max(x1, x2);
        rootCount = 2;
    }
...
double cosBetween(const dvec2& v1, const dvec2& v2) {
	double top = glm::dot(v1, v2);
    double bottom = glm::length(v1) * glm::length(v2);
    return top / bottom;
}
...
double areaOfParallelogram(const dvec3& v1, const dvec3& v2) {
	dvec3 product = glm::cross(v1, v2);
    return glm::length(product);
}
